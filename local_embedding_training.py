...
def run_word2vec(pd_map, docs, cates, folder):
    ...
    for cate in cates:
        ...
        # embed_proc.wait()
        subprocess.run(["./word2vec", "-threads", "20", "-train", input_f, "-output", output_f], shell=True)
...
def word2vec(inputFile, outputFile, SIZE=100, WINDOW=5, MIN_COUNT=5):
    '''
    ...
    try:
        model = FastText(size=SIZE, window=WINDOW, min_count=MIN_COUNT, seed=0,
...
def run_word2vec_n(pd_map, docs, cates, folder):
    for cate in cates:
        ...
        model = word2vec(input_f, output_f)
        print('[Local-embedding] done training word2vec')
...
def main_local_embedding(folder, doc_file, reidx, parent, N):
    ...
    run_word2vec_n(pd_map, docs, cates, folder)
